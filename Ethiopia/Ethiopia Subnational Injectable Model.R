# Ethiopia Subnational Injectable Model
library(tidyr)
library(dplyr)
library(jsonlite) 
library(haven)
...
setwd("C:/Users/KristinBietsch/files/Track20/Win Requests/Self Injections/Subnational/Ethiopia")
# DHS Labels
#labels <- read.csv("Nigeria DHS Labels.csv")

# FPET Runs
fpet <- read.csv("EthiopiaRegionFPETRunsAW.csv")
# Population Data
pop <- read.csv("C:/Users/KristinBietsch/files/Track20/Win Requests/Self Injections/Subnational/Ethiopia/Ethiopia Population Subnational_population.csv")
...
pop2 <- pop %>% 
  ...
  mutate(Region=case_when(Region=="Addis Adaba" ~ "AddisAdaba",
                          Region=="Afar" ~ "Afar",
                          Region=="Amhara" ~ "Amhara",
                          Region=="Benishangul" ~ "Benishangul",
                          Region=="Dire Dawa" ~ "DireDawa",
...
discontinuation<-dta %>% select( CountryName, SurveyId, Value, CharacteristicLabel)   %>% 
  ...
  mutate(discon_inj=Injectables/100,  discon_stm=(Pill)/100) %>%
  mutate(ISO=231)
...
jsondata<-fromJSON(url) 
dta<-data.table(jsondata$Data)
inconv<- dta %>%  filter(SurveyId=="ET2016DHS") %>% filter( CharacteristicLabel==  "Total"    ) %>%
  rename(Disc_Inconv=Value) %>%  mutate(ISO=231) %>%
...
distance <- filter(distance_full, ISO==231)
#########################################################################################
# Most Recent Survey
women <- read_dta("C:/Users/KristinBietsch/files/DHS Data/Ethiopia/ETIR70FL.DTA", col_select = any_of(c( "v005", "v024", "v604", "v605", "v312",  "v313", "v326")))
...
allwomen <- women %>% mutate(mcpr=case_when(v313==3 ~ 1, v313!=3 ~ 0),
                             ...
                             stm=case_when(v312==1 |  v312==5 | v312==13 |  v312==16  | v312==18 ~ 1, 
                                           v312!=1 & v312!=5 & v312!=13 & v312!=16  & v312!=18 ~ 0)) %>%
  mutate(public=case_when(v326==11 | v326==12 | v326==13 | v326==14 | v326==16 ~ 1,
                          v326==21 | v326==26 | v326==31 | v326==32 | v326==33 | v326==36 | v326==41 | v326==42 | v326==96 | v326==98 ~ 0),
...
public_source <- as.data.frame(prop.table(wtd.table(x= injectable_users$public, y=as.factor(injectable_users$Region),  weights=injectable_users$sampleweights ),2)) %>% rename(Public_Source=Freq) %>% mutate(Public_Source=Public_Source*100)  %>% filter(Var1==1) %>% mutate(Region=Var2) %>% select(Region, Public_Source)
GHC_source <- as.data.frame(prop.table(wtd.table(x= injectable_users$GHC, y=as.factor(injectable_users$Region),  weights=injectable_users$sampleweights ),2)) %>% rename(GHC_source=Freq) %>% mutate(GHC_source=GHC_source*100)  %>% filter(Var1==1) %>% mutate(Region=Var2) %>% select(Region, GHC_source)
...
write.csv(results_total, "C:/Users/KristinBietsch/files/Track20/Win Requests/Self Injections/Subnational/Ethiopia/Ethiopia Region DMPA Results 110121.csv", row.names = F)
