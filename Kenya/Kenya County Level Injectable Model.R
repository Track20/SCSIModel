# Kenya Subnational Injectable Model
library(tidyr)
library(dplyr)
library(jsonlite) 
library(haven)
...
setwd("C:/Users/KristinBietsch/files/Track20/Win Requests/Self Injections/Subnational/Kenya")
# DHS Labels
#labels <- read.csv("Nigeria DHS Labels.csv")

# FPET Runs
fpet <- read.csv("Kenya County All FPET Run.csv")
# Population Data
pop <- read.csv("C:/Users/KristinBietsch/files/Track20/Win Requests/Self Injections/Subnational/Kenya/Kenya Pop DHS Dist.csv") %>% group_by(region_code, mid_year) %>%
  summarise(Pop=sum(population_count)) %>% rename(County=region_code, Year=mid_year) %>% filter(Year>=2019) 
levels(as.factor(pop$County))
...
jsondata<-fromJSON(url) 
dta<-data.table(jsondata$Data)
inconv<- dta %>%  filter(SurveyId=="KE2014DHS") %>% filter( CharacteristicLabel==  "Total"    ) %>%
  rename(Disc_Inconv=Value) %>%  mutate(ISO=404) %>%
...
distance <- filter(distance_full, ISO==404)
#########################################################################################
# Most Recent Survey
women <- read_dta("C:/Users/KristinBietsch/files/DHS Data/Kenya/KEIR72FL.DTA", col_select = any_of(c("scounty", "v005", "v024", "v604", "v605", "v312",  "v313", "v326")))

names(women)

allwomen <- women %>% mutate(mcpr=case_when(v313==3 ~ 1, v313!=3 ~ 0),
                             inject= case_when(v312==3 ~ 1, v312!=3 ~ 0),
                             ltm=case_when(v312==2 |  v312==6 | v312==7 |  v312==11  ~ 1, v312!=2 & v312!=6 & v312!=7 & v312!=11 ~ 0 ),
                             stm=case_when(v312==1 |  v312==5 | v312==13 |  v312==14 |   v312==17  ~ 1, 
                                           v312!=1 & v312!=5 & v312!=13 & v312!=14 & v312!=17  ~ 0)) %>%
  mutate(public=case_when(v326==11 | v326==12 | v326==13  | v326==16 ~ 1,
                          v326==21 | v326==22 | v326==23 | v326==24 | v326==25 | v326==26 | v326==31 | v326==32 | v326==33 | v326==34 | v326==35 | v326==96 ~ 0),
  ...
  mutate(County=case_when(scounty==58 ~ "Baringo County",
                         scounty==64 ~ "Bomet County",
                         scounty==73 ~ "Bungoma County",
                         scounty==74 ~ "Busia County",
                         scounty==56 ~ "Elgeyo Marakwet County",
                         scounty==35 ~ "Embu County",
                         scounty==21 ~ "Garissa County",
                         scounty==83 ~ "Homa Bay County",
                         scounty==32 ~ "Isiolo County",
                         scounty==62 ~ "Kajiado County",
                         scounty==71 ~ "Kakamega County",
                         scounty==63 ~ "Kericho County",
                         scounty==45 ~ "Kiambu County",
                         scounty==13 ~ "Kilifi County",
                         scounty==43 ~ "Kirinyaga County",
                         scounty==85 ~ "Kisii County",
                         scounty==82 ~ "Kisumu County",
                         scounty==36 ~ "Kitui County",
                         scounty==12 ~ "Kwale County",
                         scounty==59 ~ "Laikipia County",
                         scounty==15 ~ "Lamu County",
                         scounty==37 ~ "Machakos County",
                         scounty==38 ~ "Makueni County",
                         scounty==23 ~ "Mandera County",
                         scounty==31 ~ "Marsabit County",
                         scounty==33 ~ "Meru County",
                         scounty==84 ~ "Migori County",
                         scounty==11 ~ "Mombasa County",
                         scounty==44 ~ "Muranga County",
                         scounty==90 ~ "Nairobi County",
                         scounty==60 ~ "Nakuru County",
                         scounty==57 ~ "Nandi County",
                         scounty==61 ~ "Narok County",
                         scounty==86 ~ "Nyamira County",
                         scounty==41 ~ "Nyandarua County",
                         scounty==42 ~ "Nyeri County",
                         scounty==53 ~ "Samburu County",
                         scounty==81 ~ "Siaya County",
                         scounty==16 ~ "Taita Taveta County",
                         scounty==14 ~ "Tana River County",
                         scounty==34 ~ "Tharaka Nithi County",
                         scounty==54 ~ "Trans Nzoia County",
                         scounty==51 ~ "Turkana County",
                         scounty==55 ~ "Uasin Gishu County",
                         scounty==72 ~ "Vihiga County",
                         scounty==22 ~ "Wajir County",
                         scounty==52 ~ "West Pokot County")) 
...
mcpr <-    as.data.frame(prop.table(wtd.table(x= allwomen$mcpr, y=as.factor(allwomen$County),  weights=allwomen$sampleweights ),2))  %>% rename(mCPR=Freq) %>% filter(Var1==1) %>% mutate(County=Var2) %>% select(County, mCPR)
inject <-    as.data.frame(prop.table(wtd.table(x= allwomen$inject, y=as.factor(allwomen$County),  weights=allwomen$sampleweights ),2))  %>% rename(inject=Freq)   %>% filter(Var1==1) %>% mutate(County=Var2) %>% select(County, inject)
stm <-    as.data.frame(prop.table(wtd.table(x= allwomen$stm, y=as.factor(allwomen$County),  weights=allwomen$sampleweights ),2))  %>% rename(stm=Freq)   %>% filter(Var1==1) %>% mutate(County=Var2) %>% select(County, stm)
ltm <-    as.data.frame(prop.table(wtd.table(x= allwomen$ltm, y=as.factor(allwomen$County),  weights=allwomen$sampleweights ),2))  %>% rename(ltm=Freq)   %>% filter(Var1==1) %>% mutate(County=Var2) %>% select(County, ltm)

methodmix <- full_join(mcpr, inject, by="County") %>% full_join(stm, by="County") %>% full_join(ltm, by="County") 

# relevant reasons for not using
relevant <-    as.data.frame(prop.table(wtd.table( x= allwomen$relevant,  y=as.factor(allwomen$County), weights=allwomen$sampleweights ),2)) %>% rename(ReasonNotUsingSI=Freq)   %>% filter(Var1==1) %>% mutate(County=Var2) %>% select(County, ReasonNotUsingSI)
...
public_source <- as.data.frame(prop.table(wtd.table(x= injectable_users$public, y=as.factor(injectable_users$County),  weights=injectable_users$sampleweights ),2)) %>% rename(Public_Source=Freq) %>% mutate(Public_Source=Public_Source*100)  %>% filter(Var1==1) %>% mutate(Region=Var2) %>% select(Region, Public_Source)
GHC_source <- as.data.frame(prop.table(wtd.table(x= injectable_users$GHC, y=as.factor(injectable_users$County),  weights=injectable_users$sampleweights ),2)) %>% rename(GHC_source=Freq) %>% mutate(GHC_source=GHC_source*100)  %>% filter(Var1==1) %>% mutate(Region=Var2) %>% select(Region, GHC_source)
# No fieldworker data
...
#########################################################################################################################
mcpr_clean <- fpet %>% filter(year>=2019) %>% rename(mcpr=contraceptive_use_modern, Year=year, County=region_code)  %>% select(County, Year, mcpr)

users_prop <- methodmix %>% mutate( prop_ltm = ltm/mCPR,
                                    prop_stm= stm/mCPR,
                                    prop_Injection= inject/mCPR) %>%
  select(County,  prop_ltm, prop_stm, prop_Injection)

data <- full_join(mcpr_clean, users_prop, by=c("County"))  %>% full_join(pop, by=c("County", "Year"))
...
total_users <- data %>% mutate(LTM = mcpr * prop_ltm * Pop,
  ...
  mutate(ISO=404) %>%
  select(ISO, County, Year, Total, LTM, STM, Injection, NonUser) 
###############################################################################
...
policy_clean <- full_join(public_source, GHC_source, by="Region") %>% mutate(Fieldworker_source=0) %>% 
  ...
  mutate(PSR=case_when(Public_Source<50 ~ 1, Public_Source >=50 & Public_Source <75 ~ 2, Public_Source>75 ~ 3),
         ...
         HC_CBR_total = replace_na(HC_CBR_total, 0),
...
baseline <- full_join(total_users, prop.inject.2019, by="County") %>%
  full_join(relevant, by="County") %>% 
  full_join(policy_clean,  by="County") %>%
  full_join(discontinuation_clean,  by="ISO") %>%
  full_join(parameters, by="ISO" ) %>%
  filter(!is.na(Year)) %>%
  mutate(CountryYear=paste(County, Year, sep="")) %>%
  rename(iso=ISO, Country=County) %>%
...
write.csv(results_total, "C:/Users/KristinBietsch/files/Track20/Win Requests/Self Injections/Subnational/Kenya/Kenya State DMPA Results 110221.csv", row.names = F)
